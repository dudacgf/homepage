...
$homepage->assign('fortuneCookie', 'N√£o tem nada aqui. passe para  pr&oacute;ima.<br /><b>-- autor an√¥nimo</b>');
$homepage->assign('displayFortune', 1);

// indica a inclus„o do form de cores
$homepage->assign('displaySelectColor', 1);

// lÍ os elementos coloridos e os pares de cores
...
// Leio as categorias da p·gina e percorro-as, incluÌndo-as no template
$pagina->lerElementos();
foreach ($pagina->elementos as $categ) {

	$descricoesCategorias[] = array('index' => $categ->posPagina, 'categoria' => $categ->descricaoCategoria);
	
	// Leio os grupos desta categoria e percorro-os, incluÌndo-os no template
	$categ->lerElementos();
	unset($grupos);
	foreach ($categ->elementos as $grupo) 
	{

		// Leio os elementos deste grupo e percorro-os, incluÌndo-os no template
		$grupo->lerElementos();
		unset($elementos);
